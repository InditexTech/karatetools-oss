...
* Sign off your commits (`Signed-off-by`) to comply with requirements.
** Use `git commit -s` (or `git commit --signoff`) to add the trailer to each commit.
** Or enable it automatically for this repository: `git config commit.signoff true`.
