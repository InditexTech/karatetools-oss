...
`java -jar boot/target/karatetools-boot-X.X.X-SNAPSHOT.jar --server.port=8888`
|NA
|no errors
...
`APP_PATH_TO_JAR="boot/target/karatetools-boot-X.X.X-SNAPSHOT.jar"`

`APP_OPTIONS="--server.port=8888"`
...
a|
`APP_HEALTH_CHECK_URL="http://localhost:8080/karatetools/health/docker"`

`curl -s -o /dev/null -w \"%{http_code}\" $APP_HEALTH_CHECK_URL`
|Status code 200

|Check docker containers are up
a|`docker ps -a --format "table {{.ID}}\t{{.Names}}\t{{.State}}\t{{.Status}}\t{{.Image}}"`
