...
`mvn clean verify -DAPP_PORT=8888 -Dkarate.env=local -Dkarate.options="-t ~@mock.templates.inline -t ~@ignore"`
...
`mvn clean verify -DAPP_PORT=8080 -Dkarate.env=local -Dkarate.options="-t ~@mock.templates.inline -t ~@ignore"`
|all tests pass
